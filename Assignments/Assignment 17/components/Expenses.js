...
const Expenses = () => {
  ...
  const handleSubmit = (e) => {
    ...
    push(expensesRef, expense)
      .then(() => setExpense({ amount: "", description: "", category: "Food" }))
  ...
  const handleDelete = (expenseId) => {
    ...
    remove(expenseRef).catch((err) => console.error("Error deleting expense:", err));
  };
  ...
  const handleDownloadCSV = () => {
    ...
    expenses.forEach(exp => {
      csvContent += `${exp.category},${exp.amount},${exp.description}\n`;
    });
